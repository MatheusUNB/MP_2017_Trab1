...
unsigned int num[6],i,j,n,first,valido=0,qtd_caracteres;
int soma=0,contavirgula=0,contnum=1;
...
soma=0;	
	...
	if (txt[j+1]=='\n'){
		...
		for(n=0;n<strlen(txt3);n++){
			for (unsigned int p = 0, posicao = 0; p < strlen(txt2); p++, posicao++){
				if (txt2[posicao] == ' '){
					posicao++;
				}
				txt2[p] = txt2[posicao];
		...
		for (i=0;i<strlen(txt3);i++){
			...
			if (contavirgula==3){
				...
				if (((txt3[i]==cdel) && (txt3[i+1]!='\n')) &&
				((txt3[i]==cdel) && (txt3[i-1]!='\n'))){
					soma=-1;
					valido=1;
				}
			}
	...
	//SEGUNDO DELIMITADOR///////////
		...
		if (txt[j+1]=='\n'){
			...
			for(n=0;n<strlen(txt5);n++){
				for (unsigned int p = 0, posicao = 0; p < strlen(txt4); p++, posicao++){
					if (txt4[posicao] == ' '){
						posicao++;
					}
					txt4[p] = txt4[posicao];
